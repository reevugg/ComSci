...
// Generate 1000 random data inputs and insert them into the table
for ($i = 0; $i < 1000; $i++) {
    // Get a unique full name
    $fullName = generateUniqueName($fullNames);
...
function generateUniqueName(&$fullNames) {
    $adjectives = ["Red", "Blue", "Green", "Yellow", "Purple"];
    $nouns = ["Apple", "Banana", "Cherry", "Orange", "Grape"];

    $adjective = $adjectives[array_rand($adjectives)];
    $noun = $nouns[array_rand($nouns)];
    $name = $adjective . " " . $noun;

    while (in_array($name, $fullNames)) {
        $adjective = $adjectives[array_rand($adjectives)];
        $noun = $nouns[array_rand($nouns)];
        $name = $adjective . " " . $noun;
    }
